...
public class Dungeon implements ClockListener {
	...
	public void setParty(int x, int y, int z, Party party) {
		...
		// R�cup�rer l'�l�ment en premier (permet de valider le triplet (x,y,z))
		final Element element = getElement(x, y, z);

		// V�rifier que le groupe peut s'y placer !
	...
	public boolean clockTicked() {
		if (!partyMoves.isEmpty()) {
			...
			if (!command.clockTicked()) {
				// L'action s'est d�clench�e. La supprimer l'entr�e de la liste
	...
	public boolean moveParty(final Move move, boolean now, final AudioClip clip) {
		// Le clip ne peut �tre nul si on appelle cette m�thode
	...
	private boolean doMoveParty(final Move move, boolean now,
		...
		// Le param�tre clip peut �tre null ou non selon la m�thode appelante
	...
	private boolean movePartyNow(Move move, AudioClip clip) {
		...
		// Note: Un d�placement ne peut modifier la direction de regard et la
		// position en m�me temps (seule une t�l�portation le peut)
		...
		// Position finale ? Le demander � l'�l�ment source (pour les escaliers
		// entre autres)
		...
		if (!destinationElement.isTraversable(party)) {
			...
			// Appliquer des d�g�ts aux champions. Champions concern�s ?
	...
	public boolean teleportParty(final Position destination, final Direction direction, final boolean silent) {
		...
		// Position traversable ?
		if (!destinationElement.isTraversable(party)) {
			// Ne doit pas arriver, c'est qu'il y a un d�faut de conception du
			// niveau
	...
	public int getActualLight() {
		// Il vaut mieux que cette m�thode soit sur la classe Dungeon (seule
