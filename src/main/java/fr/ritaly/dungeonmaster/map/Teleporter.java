...
public final class Teleporter extends FloorTile implements ClockListener,
	...
	/**
	 * Indique si l'image du t�l�porteur doit �tre invers�e (permet d'animer le
	 * t�l�porteur).
	 */
	...
	/**
	 * Indique si le t�l�porteur est silencieux. Permet de t�l�porter un groupe
	 * de champions sans qu'il s'en aper�oive.
	 */
	...
	public Teleporter(DirectionTransform directionTransform, boolean silent) {
		...
		// La destination est nulle car le t�l�porteur ne t�l�porte pas
	...
	public boolean isTraversable(Creature creature) {
		// TODO Conditionner par le fait que la cr�ature va mourir en prennant
		// le t�l�porteur
		return true;
	}
	...
	protected void afterPartySteppedOn() {
		...
		if (isEnabled()) {
			// T�l�porter le groupe sur l'endroit cible
			if (log.isDebugEnabled()) {
	...
	protected void afterProjectileArrived(Projectile projectile) {
		...
		if (isEnabled()) {
			// T�l�porter le projectile sur l'endroit cible
			if (log.isDebugEnabled()) {
	...
	private void teleportProjectile(Projectile projectile) {
		// TODO Impl�menter teleportProjectile(Projectile)
	}
	...
	protected void afterCreatureSteppedOn(Creature creature) {
		...
		if (isEnabled()) {
			// T�l�porter la cr�ature sur l'endroit cible
			if (log.isDebugEnabled()) {
	...
	private void teleportCreature(Creature creature) {
		// TODO Impl�menter teleportCreature(Creature)
	}
	...
	public boolean clockTicked() {
		if (isSilent()) {
			// T�l�porteur invisible, pas besoin de l'animer
			return false;
		}

		if (temporizer.trigger()) {
			// On inverse l'image du t�l�porteur
			mirrored = !mirrored;
		}

		// Un t�l�porteur visible a tout le temps besoin d'�tre anim�
		return true;
	}
